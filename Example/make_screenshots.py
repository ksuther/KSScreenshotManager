...
#devices to use - check the iOS Simulator under Hardware -> Device to add others, such as iPad or iPad (Retina)
devices = ['"iPhone (Retina 3.5-inch)"', '"iPhone (Retina 4-inch)"']

destination_path = os.path.abspath(sys.argv[1])
project_path = os.path.join(os.path.realpath(os.path.split(__file__)[0]), 'Example')
target_name = 'KSScreenshotManagerExample'
app_name = 'KSScreenshotManagerExample.app'
...
if __name__ == '__main__':
    ...
    app_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], '..', 'build', 'Screenshots-iphonesimulator', app_name)
